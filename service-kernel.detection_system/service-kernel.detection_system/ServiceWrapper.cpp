...
) : m_bDebugging(FALSE)
{
	...
	//m_ServiceRunningMutex = CreateMutex(NULL,FALSE,NULL);
	//if (NULL == m_ServiceRunningMutex)
	//{
	//	DebugMsg(TEXT("CreateMutex error : %d\n"), GetLastError()))
	//	//Raise exception?
	//}
}
...
ServiceWrapper::~ServiceWrapper()
{
	...
	//if (NULL != m_ServiceRunningMutex)
	//{
	//	CloseHandle(m_ServiceRunningMutex);
	//}
}
...
#endif  HANDLEREX
{
	...
	case SERVICE_CONTROL_STOP: // 1
		...
		//WaitForSingleObject(pService->m_ServiceRunningMutex,INFINITE);
		pService->m_bIsRunning = FALSE;
		//ReleaseMutex(pService->m_ServiceRunningMutex); // TO-DO: fail checks
